...
async def get_market_price(item_id, sid=0):
    ...
    url = f"https://api.arsha.io/v2/kr/item?id={item_id}"
        ...
        async with aiohttp.ClientSession(headers=headers, connector=aiohttp.TCPConnector(ssl=False)) as session:
            async with session.get(url, timeout=5) as response:
                if response.status == 200:
                    ...
                    if isinstance(data, dict) and "error" in data: return await get_fallback_value(item_id, sid)
                    if isinstance(data, dict): data = [data]
                    for item in data:
                        item_sid = int(item.get('sid', item.get('enhancementMin', 0)))
                        if item_sid == sid: 
                            p, s, c = int(item.get('basePrice', 0)), int(item.get('currentStock', 0)), int(item.get('count', 0))
                            if p > 0: save_to_cache(item_id, sid, p, s, c); return p, s, c
                            else: return await get_fallback_value(item_id, sid)
                    if data and sid == 0: 
                        p, s, c = int(data[0].get('basePrice', 0)), int(data[0].get('currentStock', 0)), int(data[0].get('count', 0))
                        if p > 0: save_to_cache(item_id, sid, p, s, c); return p, s, c
                        else: return await get_fallback_value(item_id, sid)
                else: return await get_fallback_value(item_id, sid)
    except Exception: return await get_fallback_value(item_id, sid)
...
def get_bdo_time():
    base = datetime(2026, 6, 26, 0, 0, 0, tzinfo=KST)
    now = datetime.now(KST)
    diff = (now - base).total_seconds()
    game_total_minutes = (diff * 6) 
    hour = int((game_total_minutes // 60) % 24)
    minute = int(game_total_minutes % 60)
    is_night = hour >= 22 or hour < 7
    if is_night:
        target = 7 if hour < 7 else 31
        remain_m = (target - hour) * 60 - minute
    else:
        remain_m = (22 - hour) * 60 - minute
    return f"{hour:02d}:{minute:02d}", "밤" if is_night else "낮", int(remain_m)
...
class AvgPriceModal(discord.ui.Modal, title='실시간 아이템 가격 조회'):
    item_input = discord.ui.TextInput(label='아이템 이름 (비슷하게 쳐도 됩니다)', placeholder='예: 프리오네 반지, 기파, 7210', required=True, max_length=30)
    async def on_submit(self, interaction: discord.Interaction):
        await interaction.response.defer(ephemeral=True)
        raw_input = self.item_input.value.strip()
        if raw_input.isdigit(): item_id, best_match = int(raw_input), raw_input
        else:
            if not ITEM_LIST: 
                await interaction.followup.send("DB 로드 중입니다.", ephemeral=True); return
            best_match, score = process.extractOne(raw_input, ITEM_LIST.keys())
            if score < 50: 
                await interaction.followup.send(f"'{raw_input}'을(를) 찾을 수 없습니다.", ephemeral=True); return
            item_id = int(ITEM_LIST[best_match])
            
        price, stock, _ = await get_market_price(item_id)
        if price > 0:
            embed = discord.Embed(title="실시간 거래소 가격", color=0x3498db)
            embed.description = f"**아이템:** `{best_match}`\n**가격:** **{price:,}** 은화 ({format_kr(price)})\n**매물:** {stock:,}개"
            await interaction.followup.send(embed=embed, ephemeral=True)
        else: 
            await interaction.followup.send(f"{best_match} 정보를 불러올 수 없습니다.", ephemeral=True)
...
class PearlTimeView(discord.ui.View):
    ...
    async def fetch_and_show(self, interaction: discord.Interaction, hours: int, time_label: str):
        ...
        if not ranking:
            await interaction.followup.send("DB를 수집 중입니다. (최소 10분 이후 데이터를 불러올 수 있습니다.)", ephemeral=True)
            return
    ...
    async def select_callback(self, interaction: discord.Interaction):
        ...
        if row:
            ...
            if diff.total_seconds() > 0:
                ...
                embed.description = f"**최근 처치:** {format_dt(kill_time)}\n**현재 젠 완료 상태입니다!**"
        else:
...
class DarkRiftActionView(discord.ui.View):
    ...
    @discord.ui.button(label="현재 시간으로", style=discord.ButtonStyle.success)
    async def btn_now(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        embed = discord.Embed(title="어둠의 틈", color=0x2b2d31)
        embed.description = f"현재 시간으로 설정되었습니다!\n\n**입력한 시간 (잡은 시간)**\n{format_dt(now)}\n\n**젠 예정 시간 (5일 후)**\n{format_dt(spawn_time)}\n\n**알림**\n• 1일 전\n• 3시간 전"
        await interaction.response.edit_message(embed=embed, view=DarkRiftMainView())
...
class DarkRiftManualModal(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            spawn_time = kill_time + timedelta(days=5)
            embed = discord.Embed(title="어둠의 틈", color=0x2b2d31)
            embed.description = f"시간이 수동으로 설정되었습니다!\n\n**입력한 시간 (잡은 시간)**\n{format_dt(kill_time)}\n\n**젠 예정 시간 (5일 후)**\n{format_dt(spawn_time)}\n\n**알림**\n• 1일 전\n• 3시간 전"
            await interaction.response.edit_message(embed=embed, view=DarkRiftMainView())
        except ValueError: 
...
async def setup_hook():
    global ITEM_LIST
    HARDCODED = {"카프라스의 돌": 7210, "기억의 파편": 44195, "블랙스톤 (무기)": 16001, "블랙스톤 (방어구)": 16002, "뾰족한 흑결정 조각": 16005, "단단한 흑결정 조각": 16004, "크론석": 16080, "고대 정령의 가루": 7200, "데보레카 목걸이": 11653}
    ITEM_LIST.update(HARDCODED)
    try:
        with open('items_v2.json', 'r', encoding='utf-8') as f: ITEM_LIST.update(json.load(f))
    except Exception: pass
